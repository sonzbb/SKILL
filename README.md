...
一句话：**让 Codex 继续做主脑，把长日志、长搜集、大范围扫描、重复执行这些重材料任务交给 OpenCode sidecar。**
...
```text
安装并使用 https://github.com/sonzbb/SKILL 里的 OpenCode Sidecar V2。
...
- 任务结束后只回收紧凑的 `result.md`，再由 Codex 做最终判断
...
- `.opencode-bridge/` 是真正的 V2 桥接实现
- `docs/` 里保留了 V2 的设计与计划文档
...
- 这个仓库的重点是“让 sidecar 真能被复用”，不是只保存一套本地脚本
